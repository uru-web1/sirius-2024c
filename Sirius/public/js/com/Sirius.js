export const Sirius = class extends HTMLElement {
    constructor(props) {
        super();
...
if (!customElements.get('x-sirius')) {
    customElements.define('x-sirius', Sirius);
}

window.Sirius = Sirius;
window.sirius = new Sirius();
sirius.actZIdx = 0;
sirius.radioGroup = [];
sirius.react = () => {
    for (let attr in sirius) {
        if (sirius[attr]) {
            if (sirius[attr].value || sirius[attr].equation) {
                if (sirius[attr]._equation) {
                    sirius[attr]._value = eval(sirius[attr].equation.toString());
                }
                ;
            }
        }
